...
> Última actualización: 2026-06-26 (fin de sesión Fases 0–4 núcleo).
...
- **Fase 4** (Auth + RLS): **capa DB+RLS HECHA y PROBADA (adversarial 17/17)**, pero el **WIRING de la UI de auth está PENDIENTE** (6 archivos) → **la app NO loguea por UI todavía. Fase 4 NO está cerrada.**
...
- `src/pages/POSLogin.jsx` — login (pendiente wirear a `login_pos` RPC).
...
**Bloqueante:** decisión de Miguel sobre modo empleado (cuentas "terminal" por sucursal [recomendado] vs requerir PIN). Luego: wiring de la UI de auth (6 archivos) + build + smoke del login → cierra Fase 4.
...
- Supabase staging `ivqcxdpqxwjxfohiswqb`: SOLO datos maestros (sucursales 3, categorías 8, productos 20, usuarios 33, config 1); transaccional 0. Cuenta auth por operador `<id>@pos.confetti.local` (password `POS-<pin>`). Cuenta `staging-pos@confetti.local` temporal (eliminar cuando exista el login real).
