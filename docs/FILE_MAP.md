...
|---|---|---|
| `src/components/pedidos/TicketPastelConfetti.jsx` | **Es el ticket de pastel QUE SE IMPRIME** (lo monta `PedidoPastelDetalleDialog`). Se le añadió el aviso "FAVOR DE REGRESAR LA BASE LIMPIA" al final del todo | ⚠️ **No lo confundas con `TicketPedidoPastel.jsx`**, que tiene un texto parecido y **no lo renderiza nadie** (`NuevoPedidoPastel.jsx` lo importa y nunca lo usa). Todo el layout va **en línea**: el iframe térmico no carga Tailwind |
