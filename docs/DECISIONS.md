...
22. **Folio en la pantalla Gracias del web — DECISIÓN PENDIENTE DE MIGUEL.** Consecuencia de #20: anon hace INSERT pero **no puede leer de vuelta el folio** (`pedidos` no tiene SELECT para anon — correcto por aislamiento; `insert().select()` → 42501, probado). La fila SÍ queda con folio real. Opciones para mostrarlo en Gracias: **(1, recomendada)** RPC `crear_pedido_web(payload jsonb) returns text` SECURITY DEFINER (migración 0019) que inserte y **devuelva el folio** — el web cambia `insert` por `rpc`; **(2)** Gracias sin folio (confirmación por WhatsApp), sin tocar esquema; **(3)** policy anon SELECT en `pedidos` — **descartada** (filtraría pedidos ajenos, rompe aislamiento). Se decide con Miguel al abrir WEB-2 (cambia el código del envío).
