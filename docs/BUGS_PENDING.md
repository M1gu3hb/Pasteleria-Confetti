...
## (k) ⚠️ Abono MIXTO no entra a los buckets de método → inconsistencia con el doble conteo + no sale en ResumenDelDia (FASE 3 #3, decisión de Miguel PENDIENTE)
...
- **Opciones (decisión de Miguel; NO tocado):**
  ...
  - El doble conteo es CANDADO (fidelidad Base44, que el bot validó); Base44 nunca tuvo abonos mixtos. **Miguel decide A vs B antes de tocar `efectivo_esperado`/buckets.**
