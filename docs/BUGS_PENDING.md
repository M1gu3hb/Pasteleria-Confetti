...
## (i) 🐛 Venta paralela de ANTICIPO sin `DetalleVenta` — `producto_id: ''` en columna `uuid NOT NULL` (destapado en FASE 3 #1)
- **Qué:** `RegistrarPagoDialog.jsx:108` (y `CobrarPedidoWebDialog`, y `Caja.handleCobrarPedidoWeb`) crean el `DetalleVenta` de la venta paralela con `producto_id: ''` (string vacío). La columna `detalle_venta.producto_id` es **`uuid NOT NULL`** → el insert falla con `invalid input syntax for type uuid: ""`. El error se captura (no fatal) pero **no se crea la línea**.
- **Efecto:** el **dinero del anticipo SÍ entra al corte** (la `Venta` paralela `pagada` se crea bien y suma en corte/dashboard), pero la **línea "Adelanto de pago — PP-…" NO aparece en el ticket/PDF** del corte (la venta queda sin desglose). Afecta a **TODOS los anticipos** (POS y web), no solo los web.
- **Por qué surge ahora:** estaba latente porque los pedidos web no eran cobrables (saldo=0); al arreglar el saldo en FASE 3 #1, el camino corre y el error sale (toast visible). El cuadre del bot (totales) no lo detectó porque lee `ventas`, no `detalle_venta`.
- **Fix (decisión de Miguel — toca esquema/money-path, separable del #1):** opciones — (1) hacer `detalle_venta.producto_id` nullable y usar `null` para líneas sin producto de catálogo (pago/servicio); (2) no crear `DetalleVenta` para la venta paralela y que el ticket/PDF muestre el concepto desde `venta.notas`. NO se aplicó; se reporta para Fase 3 (#2/#3, anticipos saliendo correctos en el corte).
