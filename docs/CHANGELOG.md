...
- **Firma — la hace Miguel** (regla dura: la contraseña del keystore NO está en el config de gradle → no la maneja esta sesión). Entregado **ALINEADO SIN FIRMAR** en `release/ConfettiPOS-nuevo-alineado-SIN-FIRMAR.apk` (SHA-256 `56b74b08d6c456d50d5fbc4cfd83e93b5c0bc56f4969f36629819f609c3961a5`, build reproducible: mismo hash en 2 compilaciones). Viejo (9-jul, `c8c5075…318ec`) respaldado en `release/ConfettiPOS_2026-07-09.apk` (+ `_9jul_backup.apk`). Comandos exactos de firma + verify v2/v3 en el reporte. El hash FINAL (firmado) ≠ `c8c5…` lo confirma tras firmar. `release/ConfettiPOS.apk` NO se reemplazó con el sin-firmar (quedaría no instalable): se reemplaza con el firmado.
- **Página de descarga (respaldo de la USB):** `release/descarga/index.html` — estática sin dependencias (botón "Descargar APK", versión/commit/SHA-256, 3 pasos de instalación). Se publica a Vercel (ruta con slug aleatorio, MIME `application/vnd.android.package-archive`) EN CUANTO exista el APK firmado (sirve el binario correcto, no el sin-firmar).
